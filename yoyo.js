...
window.addEventListener('DOMContentLoaded', (event) => {
    ...
    class SpringOP {
        ...
        balance() {
            ...
            this.body.xmom = (this.body.xmom + xmomentumaverage) / 2
            this.body.ymom = (this.body.ymom + ymomentumaverage) / 2
            this.anchor.xmom = ((this.anchor.xmom) + (xmomentumaverage*1)) / 2
            this.anchor.ymom = ((this.anchor.ymom) + (ymomentumaverage*1)) / 2
        }
        draw() {
            this.beam.draw()
        }
...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class YoYo{
        constructor(){
            this.ball = new Circle(350,450, 50, "white")
            this.angle = Math.PI*1.5
            this.dot = new Circle(this.ball.x+(Math.cos(this.angle)*50), this.ball.y+(Math.sin(this.angle)*50), 5, "red")
            this.anglemom = 0
            this.springs = []
            this.dots = [this.dot]
            let newdot = new Circle(this.dot.x, this.dot.y-15, 3, "blue")
            newdot.friction = .98
            this.dots.push(newdot)
            let spring = new SpringOP(this.dot, newdot, 15, 3, "#00ff00")
            this.springs.push(spring)
            for(let t = 0;t<30;t++){
                let newdot = new Circle(spring.anchor.x, spring.anchor.y-15, 3, "blue")
                newdot.friction = .98
                spring = new SpringOP(spring.anchor, newdot, 15, 3, "#00ff00")
                this.springs.push(spring)
                this.dots.push(newdot)
            }
            this.grid = []
            for(let t = 0;t<7000;t++){
                let square = new Rectangle(1400+((t%100)*7), 720+(Math.floor(t/100)*7), 7,7,"blue")
        ...
        draw(){
            ...
            for(let t=  0;t<this.grid.length;t++){
                if(this.ball.isPointInside(this.grid[t])){
                    let link = new LineOP(this.grid[t], this.ball)
                    this.grid[t].xmom = Math.cos(link.angle())*6
                    this.grid[t].ymom = Math.sin(link.angle())*6
                    this.grid[t].color = "red"
                }
                this.grid[t].move()
                this.grid[t].draw()
            }
            ...
            this.springs[this.springs.length-1].anchor.x = TIP_engine.x
            this.springs[this.springs.length-1].anchor.y = TIP_engine.y
            this.springs[this.springs.length-1].anchor.xmom = 0
            this.springs[this.springs.length-1].anchor.ymom = 0
            // if(this.dot.xmom > 0){
            //     this.anglemom +=.0005
            //     this.ball.xmom += this.dot.xmom*.005
            //     this.ball.ymom += this.dot.ymom*.005
            //     this.dot.xmom *= .9
            //     this.dot.ymom *= .9
            // }else{
            //     this.anglemom -=.0005
            ...
            // }
                ...
                this.ball.xmom += this.dot.xmom*.5
                this.ball.ymom += this.dot.ymom*.5
                this.dot.xmom *= .2
                this.dot.ymom *= .2
            // this.anglemom *= .97
            // this.angle += this.anglemom
            this.dot.x = this.ball.x+(Math.cos(this.angle)*50)
            this.dot.y = this.ball.y+(Math.sin(this.angle)*50)
            this.ball.reflect = 1
            this.ball.friction = .975
            this.ball.ymom+=.5
            this.ball.frictiveMove()
            for(let t = 0;t<this.dots.length;t++){

                this.dots[t].frictiveMove()
                if(this.dots[t].doesPerimeterTouch(this.ball)){
                    let link = new LineOP(this.dots[t], this.ball)
                    this.dots[t].x = this.ball.x+ (Math.cos(link.angle())*(this.ball.radius+this.dots[t].radius))
                    this.dots[t].y = this.ball.y + (Math.sin(link.angle())*(this.ball.radius+this.dots[t].radius))
                    this.dots[t].xmom *= .8
                    this.dots[t].ymom *= .8
                    this.ball.ymom *= .97
                    this.ball.xmom *= .97
